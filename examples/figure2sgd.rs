...
use plotly::{
    common::{color::Color, Anchor, DashType, Font, Line, Mode, Title},
    layout::{Axis, AxisType, Legend, Margin, Shape, ShapeType},
    Layout, NamedColor,
};
...
fn main() {
    ...
    let scales: Vec<f64> = (0..NUM_R_POINTS)
        .map(|i| {
            10_f64.powf(
                MIN_R.log10()
                    + i as f64 * (MAX_R.log10() - MIN_R.log10()) / NUM_R_POINTS.sub(1) as f64,
            )
        })
        .collect();
